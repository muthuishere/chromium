...
<ctx-optimize>
  ...
  <store>MULTI-MODULE repo, pre-built knowledge store at `~/ctxoptimize/chromium/` — one graph per module + a navigator, 21 modules declared in `.ctxoptimize/config.json`.</store>
  ...
  federates via the navigator (`~/ctxoptimize/chromium/navigator.md`; `--modules all|a,b` widens).
  ...
  <no-local-store>Fresh clone with nothing at `~/ctxoptimize/chromium/`? Run `ctx-optimize up` —
